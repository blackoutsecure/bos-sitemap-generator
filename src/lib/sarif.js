 ...
 * Security tab. Callers that need them should consume the JSON report.
 */
...
const TOOL_NAME = 'BOS Sitemap Generator';
...
function auditRun(findings, options = {}) {
  ...
  for (const finding of reportable) {
    ...
    rules.push({
      ...
      properties: { tags: ['seo', 'sitemap', 'blackout-secure'] },
    });
  }
  ...
  const results = reportable.map((finding) => ({
    ...
    partialFingerprints: { bosSitemapFindingKey: finding.findingKey },
    properties: {
...
function locationFor(finding, baseDir) {
  ...
  // A URL location has no meaningful file mapping; SARIF requires an
  // artifact URI, so anchor those on the repository root instead.
