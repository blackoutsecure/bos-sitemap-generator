 ...
 * token is exposed to the job, any other provider requires both an explicit
 * name and an endpoint. Any transport or authorization failure is treated
 * as ordinary unavailability so the run never fails because of AI.
 */
  ...
  if (GITHUB_PROVIDER_ALIASES.includes(name)) {
    ...
    if (token) {
      return {
        ...
        model: env.GITHUB_MODELS_MODEL_SITEMAP || env.GITHUB_MODELS_MODEL || 'openai/gpt-4o-mini',
        token,
      };
    }
    return null;
...
async function summarize(findings, provider, options = {}) {
  ...
  const payload = {
    ...
    messages: [
      {
        role: 'system',
        content: 'You are an SEO and site-hygiene triage assistant.',
      },
      {
        role: 'user',
        content:
          'Summarize these sitemap and SEO audit findings in at most three concise bullets. ' +
...
function localSummary(result) {
  ...
  if (!attention.length) {
    lines.push('- No sitemap or SEO control requires attention in this run.');
    return lines.join('\n');
  }
 ...
 * @param {object} result - An `AuditResult`.
 * @param {object} remediation - Resolved `sitemap.remediation` config.
