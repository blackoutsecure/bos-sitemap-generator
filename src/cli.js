 ...
 *
 * CLI companion to the composite Action.
 *
 * Subcommands:
 *   version   Print the package version.
 *   validate  Resolve layered configuration and print the result.
 *   audit     Run the sitemap/SEO audit against a published directory.
...
const USAGE = `bos-sitemap <command> [options]

Commands:
  version                       Print the package version.
  validate                      Resolve and validate layered configuration.
  audit                         Run the sitemap/SEO audit.
...
audit options:
  ...
  --public-dir <dir>            Published site directory (default: dist).
...
/**
 * Parse argv into a flag map plus positional arguments.
...
function resolveConfig(flags, { repoName = '' } = {}) {
...
function cmdValidate(flags) {
  ...
  const out = [
    ...
    `generate:     xml=${cfg.generate.xml} txt=${cfg.generate.txt} gzip=${cfg.generate.gzip} file=${cfg.generate.sitemapFilename}`,
    `seo:          lastmod=${cfg.seo.lastmodStrategy} changefreq=${cfg.seo.changefreq || '(unset)'} priority=${cfg.seo.priority || '(unset)'}`,
    `audit:        enable=${cfg.audit.enable} fail_on=${cfg.audit.failOn} max_url_length=${cfg.audit.maxUrlLength} min_url_count=${cfg.audit.minUrlCount}`,
    ...
    ...Object.entries(cfg.audit.rules).map(
      ([name, severity]) => `  ${name.padEnd(34)} ${severity}`,
    ),
  ];
...
async function cmdAudit(flags) {
  ...
  if (!siteUrl) {
    process.stderr.write('error: --site-url is required\n');
    return 2;
  }

  const publicDir = path.resolve(root, flags.publicDir || 'dist');
  if (!fs.existsSync(publicDir)) {
    process.stderr.write(`error: public dir not found: ${publicDir}\n`);
    return 2;
  }
  ...
  const urls = await buildUrls(
    {
      baseUrl: siteUrl,
      publicDir,
      includePatterns: [...cfg.discovery.includePatterns],
      excludePatterns: [...cfg.discovery.excludePatterns],
      excludeExtensions: [...cfg.discovery.excludeExtensions],
      excludeUrls: [...cfg.discovery.excludeUrls],
      lastmodStrategy: cfg.seo.lastmodStrategy,
      changefreq: cfg.seo.changefreq || undefined,
      priority: cfg.seo.priority || undefined,
      additionalUrls: [...cfg.discovery.additionalUrls],
      parseCanonical: cfg.discovery.parseCanonical,
      discoverLinks: cfg.discovery.discoverLinks,
    },
    consoleCore(),
  );

  const generatedFiles = [
    path.join(publicDir, cfg.generate.sitemapFilename),
    path.join(publicDir, cfg.generate.sitemapFilename.replace(/\.xml$/i, '.txt')),
...
async function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    if (err instanceof cfgMod.ConfigError) {
      process.stderr.write(`error: ${err.message}\n`);
      return 2;
    }
    process.stderr.write(`error: ${err.message}\n`);
